(() => {
  ...
  function openProject(id) {
    ...
    const excerpt = repo.sourceExcerpt ? `<p class="modal-section-title">SOURCE NOTE / README</p><div class="modal-excerpt">${escapeHTML(repo.sourceExcerpt).replaceAll("\n", "<br>")}</div>` : "";
    ...
    $("#modal-content").innerHTML = `<p class="modal-kicker" style="color:${color}">${escapeHTML(category.toUpperCase())} / PROJECT ARCHIVE</p><h2 class="modal-title" id="modal-title">${escapeHTML(repo.name)}</h2><p class="modal-fullname">${escapeHTML(repo.fullName)} · ${repo.private ? "PRIVATE" : "PUBLIC"}</p><p class="modal-description">${escapeHTML(repo.description)}</p><div class="modal-grid"><div class="modal-stat"><span>RECENT MOVES</span><b>${formatNumber(repo.meaningfulCommitCount ?? repo.recentCommitCount)}</b></div><div class="modal-stat"><span>BRANCHES</span><b>${formatNumber(repo.branchCount)}</b></div><div class="modal-stat"><span>DEFAULT</span><b>${escapeHTML(repo.defaultBranch || "—")}</b></div></div><p class="modal-section-title">LATEST SIGNAL</p>${latest}${excerpt}<p class="modal-section-title">PARALLEL UNIVERSES</p>${branches}<p class="modal-section-title">RECENT MOVES</p><div class="modal-recent-list">${recentHtml}</div><div style="margin-top:26px"><a class="button button-outline" href="${escapeHTML(repo.latestCommit?.url || "#")}" target="_blank" rel="noreferrer">在 GitHub 打开 <span>↗</span></a></div>`;
